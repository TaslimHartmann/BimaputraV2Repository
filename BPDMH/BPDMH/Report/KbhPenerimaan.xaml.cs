...
{
    /// <summary>
    /// Interaction logic for KbhPenerimaan.xaml
    /// </summary>
    public partial class KbhPenerimaan
    {
        ...
        private void PenerimaanReport_OnLoaded(object sender, RoutedEventArgs e)
        {
            GetCabangLookup();
            ...
            else
            {
                ...
                foreach (var getThResult in getThResults.ToList())
                {
                    var daftarMuatHarian = new Model.KbhPenerimaan()
                    {
                        ...
                        RtrTerima = GetReturTerima(getThResult)
                    };
        ...
        private List<GetRtrTerimaByPengIdResult> GetReturTerima(GetTrnPenerimaanHResult getThResult)
        {
            var tdResult = _bpdmhContext.GetRtrTerimaByPengId(getThResult.TrnPenerimaanId);
            return tdResult.Select(getThResultForDetail => new GetRtrTerimaByPengIdResult
            {
                ReturTrnTerimaId = getThResultForDetail.ReturTrnTerimaId, 
                PenerimaanId = getThResultForDetail.PenerimaanId, 
        ...
        private void BtnCetak_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            dlg.FileName = "KBHPenerimaan_" + DateTime.Now.ToString(format, CultureInfo.CreateSpecificCulture("id-ID"));
            dlg.DefaultExt = ".text";
        ...
        private void PrintToExcel(string fileName)
        {
            ...
            xlWorkSheet.Cells[1, 1] = "KBH TERIMA";
            ...
            for (var x = 0; x < daftarMuatHarians.Count; x++)
            {
                ...
                a++;
                var getRtrTerimaByPengIdResults = daftarMuatHarians[x].RtrTerima;
                if (getRtrTerimaByPengIdResults.Count > 0)
                    xlWorkSheet.Cells[rowIndex, a + 5] = getRtrTerimaByPengIdResults[0].Penerima;
